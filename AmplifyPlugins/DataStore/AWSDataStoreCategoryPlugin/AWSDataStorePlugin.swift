...
final public class AWSDataStorePlugin: DataStoreCategoryPlugin {
    ...
    /// `true` if the corresponding `.ready` event has been emitted
    var isDataStoreReady: AtomicValue<Bool>
    ...
    public init(modelRegistration: AmplifyModelRegistration,
        ...
        self.isDataStoreReady = AtomicValue(initialValue: false)
    ...
    private func registerHubListener() {
        hubListener = Amplify.Hub.listen(to: .dataStore) { hubPayload in
            if hubPayload.eventName == HubPayload.EventName.DataStore.ready {
                self.isDataStoreReady.set(true)
            }
        }
    }
